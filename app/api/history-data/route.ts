import { db, monthHistories, yearHistories } from "@/db";
...
async function getYearHistoryData(userId: string, year: number) {
  const result = await db.query.yearHistories.findMany({
    ...
    orderBy: (yearHistory, { asc }) => [asc(yearHistory.month)],
  });
...
async function getMonthHistoryData(
  ...
  const result = await db.query.monthHistories.findMany({
    ...
    where: and(
      ...
      eq(monthHistories.year, year),
      eq(monthHistories.month, month)
    ),
    orderBy: (monthHistory, { asc }) => [asc(monthHistory.day)],
  });
